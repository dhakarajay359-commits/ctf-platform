...
for (const c of challenges) {
  const exists = await db.prepare('SELECT 1 FROM challenges WHERE title = ?').get(c.title);
  if (!exists) {
...
console.log('Successfully injected ' + challenges.length + ' challenges across all categories!');
  process.exit(0);
})();