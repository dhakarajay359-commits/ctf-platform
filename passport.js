    ...
    passport.use(new GoogleStrategy({
      ...
      try {
        ...
        if (!team) {
          ...
          const info = await db.prepare('INSERT INTO teams (name, password_hash, google_id, operative_type, members_count) VALUES (?, ?, ?, ?, ?)').run(teamName, '', profile.id, 'Lone Wolf', 1);
          team = {
            id: info.lastInsertRowid,
            name: teamName,
            google_id: profile.id,
            operative_type: 'Lone Wolf',
            members_count: 1
          };
        }
        return cb(null, team);
