...
router.get('/heat', async (req, res) => {
  ...
  const challenges = await db.prepare(`
    ...
    WHERE c.visible = 1
  `).all();
  const solves = await db.prepare('SELECT challenge_id, COUNT(*) as count FROM solves GROUP BY challenge_id').all();
  const solveMap = new Map(solves.map(s => [s.challenge_id, s.count]));
  const attempts = await db.prepare('SELECT challenge_id, COUNT(*) as count FROM wrong_attempts GROUP BY challenge_id').all();
